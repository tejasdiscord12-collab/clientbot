...
if (!fs.existsSync(dbPath)) {
    fs.writeFileSync(dbPath, JSON.stringify({ warns: [], tickets: [], customCommands: [], welcomeSettings: [], statusSettings: [], invites: [], giveaways: [], ticketSettings: [], transcriptSettings: [], reviewSettings: [], memberInvites: [] }, null, 4));
}
...
module.exports = {
    ...
    setTranscriptChannel: (guildId, channelId) => {
        const db = readDB();
        if (!db.transcriptSettings) db.transcriptSettings = [];
        const index = db.transcriptSettings.findIndex(s => s.GuildID === guildId);
        if (index !== -1) {
            db.transcriptSettings[index].ChannelID = channelId;
        } else {
            db.transcriptSettings.push({ GuildID: guildId, ChannelID: channelId });
        }
        writeDB(db);
    },
    getTranscriptChannel: (guildId) => {
        const db = readDB();
        if (!db.transcriptSettings) return null;
        const settings = db.transcriptSettings.find(s => s.GuildID === guildId);
        return settings ? settings.ChannelID : null;
    },
    ...
    setReviewChannel: (guildId, channelId) => {
        const db = readDB();
        if (!db.reviewSettings) db.reviewSettings = [];
        const index = db.reviewSettings.findIndex(s => s.GuildID === guildId);
        if (index !== -1) {
            db.reviewSettings[index].ChannelID = channelId;
        } else {
            db.reviewSettings.push({ GuildID: guildId, ChannelID: channelId });
        }
        writeDB(db);
    },
    getReviewChannel: (guildId) => {
        const db = readDB();
        if (!db.reviewSettings) return null;
        const settings = db.reviewSettings.find(s => s.GuildID === guildId);
        return settings ? settings.ChannelID : null;
    },
    ...
    addMemberInvite: (guildId, memberId, inviterId) => {
        const db = readDB();
        if (!db.memberInvites) db.memberInvites = [];
    ...
    getMemberInviter: (guildId, memberId) => {
        const db = readDB();
        if (!db.memberInvites) return null;
